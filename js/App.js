//Your IPFS api key in pinata
const pinataApiKey = "6a858e1e86b1807f5710";
//Your api secret in pinata
const pinataSecretApiKey = "ed828c2cb388a799b7f1dbc0262327855691277faacc1976b14db478fa1f99b3";
window.CONTRACT = {
  address: "0x31cedb08b464ef545277b659be9408a279772b1b",
  network: "https://rpc.amoy.polygon.network",
  explore: "https://explorer.amoy.polygon.network/tx/",
  // Your Contract ABI
  abi:[
    {
      "inputs": [
...
// Use the JWT token from Pinata for authentication
const pinataJwtToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..EPuG6tWEJUubBltH2gg-vUjxmiSzPRqZ2cx_0bqVM4M"; // Replace with your JWT token
...
async function uploadFileToIpfs() {
  ...
  try {
    // Make POST request to upload the file to Pinata using Bearer token
    const response = await fetch("https://api.pinata.cloud/pinning/pinFileToIPFS", {
      method: "POST",
      body: formData,
      headers: {
        Authorization: `Bearer ${pinataJwtToken}`, // Use Bearer token for authentication
      },
    });
...
async function sendHash() {
  try {
    ...
    // Ensure get_ChainID is defined and working properly
    await get_ChainID(); // If this function is asynchronous, make sure to await it

    // Upload file to IPFS and get the CID (Content Identifier)
    const CID = await uploadFileToIpfs();

    // Verify if window.hashedfile exists and has the correct length
    if (window.hashedfile && window.hashedfile.length > 4) {
      // Call smart contract method to add the document hash and CID
      const gasLimit = 3000000; // Adjust gas limit as needed
      const gasPrice = await window.web3.eth.getGasPrice(); // Fetch the current gas price
      ...
      await window.contract.methods
        ...
        .send({
          ...
          gas: gasLimit,
          gasPrice: gasPrice, // You can also set a static price if desired
        })
        ...
        .on("receipt", function (receipt) {
          printUploadInfo(receipt); // Display receipt information
          generateQRCode(); // Generate QR code for the CID
        })
        .on("confirmation", function (confirmationNr) {
          // Handle confirmation if needed
          console.log(`Transaction confirmed: ${confirmationNr}`);
        })
      ...
      throw new Error("Invalid file hash detected.");
    }
  } catch (error) {
  ...
  if (info && address) {
    ...
    try {
      await window.contract.methods
        ...
        .send({ 
          from: window.userAddress,
          gas: 3000000 // Ensure this is enough for the transaction
        })
...
async function getExporterInfo() {
  try {
    const result = await window.contract.methods
      ...
      .call({ from: window.userAddress });
    window.info = result;
    console.log("Exporter info:", result);
  } catch (error) {
    console.error("Error getting exporter info:", error.message);
  }
}
